...
printHop h w = "The word " ++ prettyShow w ++ (if thd3 h == Aatat then "" else " turns into " ++ prettyShow (applyTransformation (thd3 h) w) ++ " under transformation " ++ show (thd3 h) ++ ". This") ++ " has value " ++ show (applyHop h w) ++ " in Mispar " ++ show (snd3 h) ++ if fst3 h == Shaveh then "." else " after " ++ explainPostProcess (fst3 h) ++ "."

wordToWord :: HFWord -> HFWord -> [String]
wordToWord w1 w2 = map (\(h1, h2) -> printHop h1 w1 ++ "\n" ++ printHop h2 w2) $ oneHop w1 w2